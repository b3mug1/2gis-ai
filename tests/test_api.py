...
class FakeAuthService:
    ...
    async def login(self, payload):  # noqa: ANN001
        user = UserResponse(id=uuid4(), email=payload.email, full_name="Test User", role="user", is_active=True)
        tokens = AuthTokens(access_token="access", refresh_token="refresh", expires_in=1800)
        return type("Result", (), {"user": user, "tokens": tokens})()

    async def refresh_session(self, refresh_token: str):
        user = UserResponse(id=uuid4(), email="user@example.com", full_name="Test User", role="user", is_active=True)
...
async def test_search_endpoint_with_dependency_overrides() -> None:
    ...
    from cityguide_backend.api.dependencies import get_current_user, get_search_service, get_auth_service

    app.dependency_overrides[get_search_service] = lambda: FakeSearchService()
    app.dependency_overrides[get_current_user] = lambda: type("User", (), {"id": uuid4(), "email": "u@example.com", "full_name": "User", "role": type("Role", (), {"value": "user"})(), "is_active": True})()
    app.dependency_overrides[get_auth_service] = lambda: FakeAuthService()

    try:
        async with AsyncClient(transport=ASGITransport(app=app, lifespan="off"), base_url="http://test") as client:
            response = await client.post("/search", json={"query": "sushi near astana", "coordinates": {"latitude": 51.0, "longitude": 71.0}}, headers={"Authorization": "Bearer test"})
    finally:
        app.dependency_overrides.clear()
