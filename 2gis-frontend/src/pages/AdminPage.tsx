    ...
    <div className="max-w-6xl mx-auto px-6 py-8 space-y-10 pb-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        >
          ...
          <p className="text-[11px] text-[hsl(var(--primary))] font-semibold flex items-center gap-1">
      ...
      <div className="space-y-4 border-t border-[hsl(var(--border))] pt-8">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Автоматическая проверка PostgreSQL, JWT токенов, кэша Redis, ИИ-модели и 2GIS навигатора
            </p>
          </div>
        ...
        {runningTests ? (
          <div className="p-8 text-center bg-card rounded-2xl border border-[hsl(var(--border))] space-y-3">
            ...
            <p className="text-xs text-muted-foreground">Проверка asyncpg, JWT, bcrypt, AI Engine, 2GIS API и OSRM</p>
          </div>
        ) : testResults.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 rounded-2xl border border-[hsl(var(--border))] bg-card flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-foreground">City Guide AI Engine</p>
                <p className="text-xs text-muted-foreground">Нейросетевая модель поиска и рекомендаций</p>
              </div>
            </div>
            <span className="text-xs font-bold text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-full flex items-center gap-1 border border-emerald-500/20">
              <CheckCircle2 className="w-3.5 h-3.5" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-5 rounded-2xl border border-[hsl(var(--border))] bg-card shadow-lg space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-bold bg-[hsl(var(--primary)/0.1)] text-[hsl(var(--primary))] border border-[hsl(var(--primary)/0.2)] px-2 py-0.5 rounded-md">
                    {emailPct}%
                  </span>
                </div>
                <div className="text-2xl font-extrabold text-foreground">{authStats.email} чел.</div>
                <div className="w-full bg-muted h-2 rounded-full overflow-hidden">
                  <div className="bg-[hsl(var(--primary))] h-full transition-all duration-500" style={{ width: `${emailPct}%` }} />
                </div>
              </div>
                    ...
                    <tr key={u.id} className="hover:bg-muted/40 transition-colors">
                      <td className="px-6 py-4 font-bold text-foreground flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center font-extrabold text-black ${
                            u.role === "admin" ? "bg-amber-500" : "bg-[hsl(var(--primary))]"
                          }`}
                        >
                      ...
                      <td className="px-6 py-4">
                        {u.oauth_provider === "google" ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-[hsl(var(--primary)/0.15)] text-[hsl(var(--primary))] border border-[hsl(var(--primary)/0.3)]">
