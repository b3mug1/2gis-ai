  ...
  return (
    <div className="premium-shell flex h-screen overflow-hidden bg-[hsl(var(--background))] text-[hsl(var(--foreground))]">
      <div className="relative flex min-w-0 flex-1 flex-col border-r border-[hsl(var(--border))] bg-[hsl(var(--background))]">
        <div
          className="border-b border-[hsl(var(--border))] px-4 py-3.5 backdrop-blur-2xl sm:px-6"
          style={{ backgroundColor: "hsl(var(--background) / 0.85)" }}
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[hsl(var(--card))] border border-[hsl(var(--border))] text-[hsl(var(--primary))] shadow-sm">
                  <MessageSquare className="h-3.5 w-3.5" />
                </div>
                <h1 className="text-sm font-bold tracking-tight text-foreground">{t.chat.title}</h1>
                <span className="rounded-full border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-2 py-0.5 text-[10px] font-semibold text-[hsl(var(--muted-foreground))]">
                  Astana
                </span>
              </div>
              <p className="mt-0.5 text-xs text-muted-foreground">
                Умный поиск заведений с анализом отзывов 2GIS
              </p>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={cn(
                  ...
                  showFilters
                    ...
                    : "border-[hsl(var(--border))] bg-[hsl(var(--card)/0.7)] text-muted-foreground hover:text-foreground"
                )}
              >
                <span>Фильтры</span>
                {(filters.open_now || filters.min_rating > 0 || filters.travel_mode || filters.price_category) && (
              ...
              {messages.length > 0 && (
                <button
                  onClick={clearChat}
                  className="btn-minimal btn-minimal-secondary px-3 py-1.5 text-xs"
                >
              ...
              <button
                ...
                className={cn(
                  "inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-semibold transition-all xl:hidden",
                  showMap
                    ? "border-[hsl(var(--primary))] bg-[hsl(var(--card))] text-[hsl(var(--primary))]"
                    : "border-[hsl(var(--border))] bg-[hsl(var(--card)/0.7)] text-muted-foreground hover:text-foreground"
                )}
          ...
          <div className="mt-3 flex flex-wrap items-center gap-1.5">
            ...
            <button
              onClick={() =>
                setFilters((f) => ({
                  ...f,
                  travel_mode: f.travel_mode === "walking" ? null : "walking",
                }))
              }
              className={cn(
                "rounded-full border px-3 py-1 text-xs font-semibold transition-all",
            ...
            <button
              onClick={() =>
                setFilters((f) => ({
                  ...f,
                  travel_mode: f.travel_mode === "driving" ? null : "driving",
                }))
              }
              className={cn(
                "rounded-full border px-3 py-1 text-xs font-semibold transition-all",
      ...
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className={cn("hidden xl:block w-[460px] shrink-0 p-3", showMap && "!block")}
      >
        <div className="surface-panel flex h-full flex-col overflow-hidden">
          <div className="flex items-center justify-between border-b border-[hsl(var(--border))] px-4 py-3 bg-[hsl(var(--card)/0.6)]">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs font-bold text-foreground">Интерактивная карта</p>
                <p className="text-[10px] text-muted-foreground">
                  {places.length > 0 ? `Отображено ${places.length} заведений` : "Астана · 2GIS Live"}
                </p>
              </div>
            </div>
            <span className="rounded-full border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-2.5 py-1 text-[10px] font-semibold text-[hsl(var(--muted-foreground))]">
              2GIS Maps
            </span>
          </div>
