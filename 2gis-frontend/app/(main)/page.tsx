...
import { Search, Sparkles, TrendingUp, Clock, ArrowRight, MapPin, Utensils, Coffee, Beer, Pizza, Cake, Heart, UtensilsCrossed } from "lucide-react";
...
const CATEGORIES = [
  { icon: UtensilsCrossed, label: "Рестораны", color: "from-orange-500 to-rose-500" },
  { icon: Coffee, label: "Кафе", color: "from-amber-500 to-orange-500" },
  { icon: Beer, label: "Бары", color: "from-yellow-500 to-amber-500" },
  { icon: Pizza, label: "Фастфуд", color: "from-red-500 to-rose-500" },
  { icon: Cake, label: "Пекарни", color: "from-pink-500 to-rose-400" },
  { icon: Heart, label: "Здоровая еда", color: "from-emerald-500 to-teal-500" },
];
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-6 pt-10 pb-28">
        ...
        >
          ...
          >
            ...
            Умный ИИ-поиск заведений в Астане
          </motion.div>

          <h1 className="text-4xl sm:text-6xl font-extrabold tracking-tight leading-[1.15] mb-5 text-foreground">
            Найдите <span className="gradient-text">идеальное место</span>
            <br />
            в Астане
          </h1>
          <p className="text-muted-foreground text-base sm:text-lg max-w-lg mx-auto leading-relaxed">
            Задавайте вопросы на обычном языке. ИИ подберет, оценит и покажет на карте лучшие заведения под ваш бюджет.
          </p>
        </motion.div>
        ...
        >
          <div className="flex flex-wrap gap-2.5 justify-center">
            {POPULAR_PROMPTS.map((promptText) => (
              <button
                key={promptText}
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {CATEGORIES.map((cat) => {
              ...
              return (
                ...
                >
                  <div className={`w-11 h-11 rounded-2xl bg-gradient-to-br ${cat.color} flex items-center justify-center text-white shadow-sm group-hover:scale-110 transition-transform duration-200`}>
